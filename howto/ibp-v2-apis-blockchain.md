...
You can only import CA, peer, and ordering nodes that are exported from other {{site.data.keyword.blockchainfull_notm}} Platform consoles running on {{site.data.keyword.cloud_notm}}, OpenShift Container Platform, Red Hat Open Kubernetes Distribution, or any Kubernetes v1.22 - v1.24 container platform on x86_64. The platform is also supported on LinuxONE (s390x) using OpenShift Container Platform.
